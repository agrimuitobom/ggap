...
const PesticideUsesList = () => {
  ...
  const [period, setPeriod] = usePeriod('pesticideUses');
  ...
  const fetchPesticideUses = useCallback(async () => {
    ...
    try {
      ...
      firestoreLogger.error('農薬使用記録の取得に失敗しました', { organizationId: currentOrganization.id }, err);
  ...
  }, [currentOrganization, fetchPesticideUses]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      firestoreLogger.error('農薬使用記録の削除に失敗しました', { pesticideUseId: id }, err);
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4">農薬使用記録一覧</h1>
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">農薬使用記録一覧</h1>
        <Link 
          to="/pesticide-uses/new" 
      ...
      {pesticideUses.length > 0 ? (
        <div className="overflow-x-auto bg-white rounded-lg shadow">
          <table className="min-w-full table-auto">
            <thead>
              <tr className="bg-gray-100">
                <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">散布日</th>
                ...
                <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">対象病害虫</th>
                <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">希釈倍率</th>
                <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">散布量</th>
                <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">散布方法</th>
                <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">天候</th>
            ...
            <tbody>
              {pesticideUses.map((use) => (
                <tr
                  ...
                  <td className="py-3 px-4 whitespace-nowrap">{use.targetPest || '-'}</td>
                  <td className="py-3 px-4 whitespace-nowrap">{use.dilutionRate ? `${use.dilutionRate}倍` : '-'}</td>
                  <td className="py-3 px-4 whitespace-nowrap">{use.amount ? `${use.amount} ${use.unit}` : '-'}</td>
                  <td className="py-3 px-4 whitespace-nowrap">{use.method || '-'}</td>
                  <td className="py-3 px-4 whitespace-nowrap">
                    <div>
                      <div>{use.weather || '-'}</div>
                      {use.temperature && (
                        <div className="text-xs text-gray-500">{use.temperature}℃</div>
                      )}
                      {use.windSpeed && (
                        <div className="text-xs text-gray-500">{use.windSpeed}m/s</div>
                      )}
                    </div>
                  </td>
        ...
        <div className="bg-white p-8 rounded-lg shadow text-center">
          <p className="text-gray-500 mb-4">農薬使用記録のデータがありません。</p>
          <Link 
            to="/pesticide-uses/new" 
            className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700"
          >
...
export default PesticideUsesList;