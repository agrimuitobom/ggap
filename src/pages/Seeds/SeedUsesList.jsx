...
const SeedUsesList = () => {
  ...
  const fetchSeedUses = async () => {
    ...
    try {
      ...
      firestoreLogger.error('播種・定植記録の取得に失敗しました', {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await moveToTrash('seedUses', id, currentOrganization.id, userProfile?.name);
      setSeedUses(seedUses.filter(use => use.id !== id));
      setDeleteConfirm(null);
    } catch (err) {
      firestoreLogger.error('播種・定植記録の削除に失敗しました', { seedUseId: id }, err);
